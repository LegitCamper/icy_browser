...
use super::categorical_selection_list::Catalog;
    ...
    Theme: Catalog,
{
    ...
    /// Options pointer to hold all items
    pub options: &'a [T],
    ...
    pub on_selected: Rc<dyn Fn(usize, T) -> Message + 'static>,
    /// The padding Width
    pub padding: Padding,
    /// The Text Size
    pub text_size: f32,
    ...
    fn draw(
        ...
        renderer.fill_text(
            iced::advanced::text::Text {
                content: self.category_name.to_string(),
            ...
            theme
                .style(&self.class, iced_aw::style::Status::Active)
                .text_color,
            bounds,
        );
        ...
        for i in start..end.min(self.options.len()) {
            ...
            if (is_selected || is_hovered) && (bounds.width > 0.) && (bounds.height > 0.) {
                renderer.fill_quad(
                    ...
                    if is_selected {
                        theme
                            .style(&self.class, iced_aw::style::Status::Selected)
                            .background
                    } else {
                        theme
                            .style(&self.class, iced_aw::style::Status::Hovered)
                            .background
                    },
                );
            }
