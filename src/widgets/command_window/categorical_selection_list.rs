...
pub use iced_aw::style::{
    selection_list::{Catalog, Style},
    Status, StyleFn,
};
use std::{fmt::Display, hash::Hash, marker::PhantomData, rc::Rc};

/// Fork of iced_aw::selection_list::List, but with category name field
use super::categorical_list::List;
...
where
    ...
    Theme: 'a
        + Catalog<
            Class<'a> = std::boxed::Box<
                dyn std::ops::Fn(
                    &'a Theme,
                    iced_aw::style::Status,
    ...
    [T]: ToOwned<Owned = Vec<T>>,
{
    ...
    pub fn new(
        ...
        let mut lists: Vec<Element<Message>> = Vec::new();
        for (category, list) in options {
            lists.push(
                List {
        ...
        let container = Container::new(Scrollable::new(Column::from_vec(lists))).padding(1);
        ...
        <Theme as Catalog>::Class<'a>: From<StyleFn<'a, Theme, Style>>,
    {
        ...
        for (category_name, options) in options {
            lists.push(
                List {
                    category_name,
                    options,
                    ...
                    text_size,
                    padding,
                    class: (Box::new(style.clone()) as StyleFn<'a, Theme, Style>).into(),
                    selected,
